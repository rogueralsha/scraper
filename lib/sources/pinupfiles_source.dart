...
class PinupfilesSource extends ASource {
  ...
  static final RegExp _videoDownloadRegExp = new RegExp(r"([^(]+)\(([\d.]+) MB\)", caseSensitive: false);
  ...
  PinupfilesSource(SettingsService settings) : super(settings) {
    this.urlScrapers
      ...
      ..add(new UrlScraper(_photosRegExp, this.emptyPageScraper, this.downloadScenes));
  }

  Future<Null> downloadScenes(String url, Document doc) async {
    _log.finest("start downloadScenes($url, $doc)");
    // Photo download
    final List<OptionElement> eles =  doc.querySelectorAll("span.select_download option");
    if(eles.isNotEmpty) {
    ...
    // Video download
    final List<AnchorElement> videoDownloadEles =  doc.querySelectorAll("ul.downloaddropdown li a");
